$(function() {
  ...
  const contentArray = [
    ...
    `<h2>Code Examples</h2><h3>Breakout</h3><p>My take on the arcade classic, this was created using HTML5 canvas, ES6 JS, and minimal CSS. Designed with cross-platform users in mind, the game scales to screen size. In addition, the modular approach makes it ideal as a template for future 2D canvas-based game development. A big "thanks" to <a href="https://github.com/jakesgordon/javascript-breakout/" target="_new">Jake Gordon</a> for the wonderful level maps!</p><p>Click to <a href="/breakout/" target="_new">play</a> or <a href="https://github.com/cmcclister/breakout" target="_new">get code</a></p><h3>Flickr Map</h3><p>Inspired by Jonny MacEachern’s <a href="https://jonny.me/tweetmap/" target="_new">Tweet Map</a>, Flickr Map integrates recent Flickr posts with Google Maps to allow users to search for public photos by location and Flickr tag. Built using ES6 JS and Bootstrap CSS styling, users can customize the look by selecting from a multitude of <a href="https://snazzymaps.com/" target="_new">Snazzy Map</a> styles.</p><p>Click to <a href="/flickr-map/" target="_new">try it</a> or <a href="https://github.com/cmcclister/flickr-map" target="_new">get code</a></p><h3>Pathfinder Alliances</h3><p>A full-stack LAMP implementation, this site features a backend with a native CMS solution and registered user tools.</p><p>Click to <a href="http://pathfinderalliances.com/" target="_new">view</a></p>`,
    ...
    `<h2>Click <a href="#" onClick="$('#contact').modal(); $('#contact').css({ bottom: $(window).height()*.66, left: $(window).width()*.10 })">here</a> to contact me</h2><p><hr /></p><h2>My Skills</h2><ul><li>AJAX/REST</li><li>Angular</li><li>Bootstrap</li><li>CSS3 (LESS/SASS)</li><li>HTML5</li><li>JavaScript (ES6)/jQuery</li><li>JSON</li></ul><ul><li>MongoDB</li><li>MySQL</li><li>Node/NPM</li><li>PHP</li><li>React</li><li>UX/UI (Responsive & Cross-browser)</li><li>Version Control (Git & SVN)</li></ul><br />&nbsp;`
  ];
  ...
  if (isEdge || isIE) { // Recommend MS browser users to switch to Chrome instead
    $('main').after('<div class="jquery-modal blocker current"><div id="msMsg" class="modal"><p>This site and its applications are not optimized for Microsoft browsers. May I recommend <a href="https://www.google.com/chrome/browser/" target="_new">Chrome</a> instead?</p></div></div>');
    $('#msMsg').modal();
